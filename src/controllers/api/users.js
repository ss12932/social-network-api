...
try {
  ...
  return res.status(500).json({success: false, error: "Failed to get all users"})
...
try {
  ...
  return res.status(500).json({success: false, error: "Failed to get single user"})
}
...
const updateUserById = (req, res) => {
  try {
    ...
    return res.status(500).json({ success: false, error: "Failed to update user by Id" });
  }
};
